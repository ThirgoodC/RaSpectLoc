...
typedef struct
{
  ...
  // Distance to the nearest cell with label X (1=WALL, 8=DOOR, 9=WINDOW ... with the new materials)
  // Distances are indexed as: 0=WALL, 1=DOOR, 2=WINDOW
  double label_dist[6];
...
//Check if motion has collision
int map_check_collision(map_t *map, double ox, double oy, double oa, double dx, double dy, double da, double* distToDoor);
...
void map_update_cspace(map_t *map, int type, double max_dist);

//update label cspace distances
